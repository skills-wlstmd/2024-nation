...
data "aws_iam_policy_document" "trail" {
  statement {
    ...
    condition {
      ...
      values   = ["arn:${data.aws_partition.trail_current.partition}:cloudtrail:${data.aws_region.trail_current.name}:${data.aws_caller_identity.trail_current.account_id}:trail/wsc2024-CT"]
    }
  }
